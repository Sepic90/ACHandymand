// Danish Postal Code Database
// Complete list of Danish postal codes with city names
 ...
 * @param {string} postalCode - 4-digit Danish postal code
 * @returns {string|null} - City name or null if not found
 */
export function getCityFromPostalCode(postalCode) {
  if (!postalCode) return null;
  
  // Remove any spaces and ensure it's 4 digits
  const cleanedCode = postalCode.replace(/\s/g, '');
  
  if (cleanedCode.length !== 4 || !/^\d{4}$/.test(cleanedCode)) {
    return null;
  }
  
  return DANISH_POSTAL_CODES[cleanedCode] || null;
}

/**
 * Format full address
 * @param {string} streetAddress - Street name and number
 * @param {string} postalCode - Postal code
 * @param {string} city - City name
 * @returns {string} - Formatted address
 */
export function formatFullAddress(streetAddress, postalCode, city) {
 ...
 * @returns {boolean} - True if valid Danish postal code
 */
export function isValidPostalCode(postalCode) {
  if (!postalCode) return false;
  const cleanedCode = postalCode.replace(/\s/g, '');
  return cleanedCode.length === 4 && /^\d{4}$/.test(cleanedCode) && DANISH_POSTAL_CODES[cleanedCode] !== undefined;
}